...
/**								 **/
/**   <Yuri van Steen Vidal>		  <15493440>   <matutino> **/
/**								 **/
/*********************************************************************/
...
int buscarPalavra_auxiliar(PONT atual, char* palavra, int n, int i) {

    //checa se ainda ha letras em palavra para percorrer, se o no atual tem algum filho e por fim se tem um filho referente a letra que buscamos
    if ( palavra[i]&& atual->filhos && atual->filhos[(palavra[i] - VALOR_a)])
    {
        //caminha para o proximo filho referente a i-esima letra em palavra
        PONT prox = atual->filhos[(palavra[i] - VALOR_a)];
        i++;
        int retorno = buscarPalavra_auxiliar(prox, palavra, n, i);
        //caso tenhamos chegado na ultima letra retornamos o contador, caso nao, retornamos zero
        if (i == n)
        {

            return prox->contador;
        }
        else if (retorno == 0)
        {
            return 0;
...
int buscarPalavra(PONT raiz, char* palavra, int n){
  ...
  return buscarPalavra_auxiliar(raiz, palavra, n, 0);;

}

void inserir_auxiliar(PONT atual, char* palavra, int n, int i) {

    //checa se nao ha nenhum filho e se ainda nao chegamos ao fim da palavra (um depois da ultima letra), seguindo a iteracao por palavra
    if (!atual->filhos && palavra[i])
    {
        //aloca espaco para um adicionar um filho, como nao ha nenhum
        ...
        //liga o atual para seu novo filho (liga as letras)
        PONT prox = criarNo();

        atual->filhos[(palavra[i] - VALOR_a)] = prox;

        i++;

        inserir_auxiliar(prox, palavra, n, i);
        //acrescentamos o contador caso esse seja a ultima letra da palavra
        if (i == n)
        {
            prox->contador++;
    ...
    //procuramos saber se o atual ja tem filhos e ainda ha letras na palavra para percorrer
    else if (atual->filhos && palavra[i])
    {
        //seguimos com a recursao caso a letra ja tenha sido previamente alocada, caso tambem seja a ultima letra, acrescentamos o contador
        ...
        //Caso nao tenha um filho correspondente a letra em palavra, cria um novo filho para essa letra, caso tambem seja a ultima letra, acrescentamos o contador
        else
        {
            PONT prox = criarNo();
...
void inserir(PONT raiz, char* palavra, int n){
    ...
    inserir_auxiliar(raiz, palavra, n, 0);

}

int excluirTodas_auxiliar(PONT atual, char* palavra, int n, int i){

    /*
    Checa se ainda ha um no em certo i e, se i for n, zeramos o contador e retornamos falso caso o no tenha filhos e verdadeiro caso tenha
    */
    if (!atual)
    {
        return false;
    ...
    int retorno = excluirTodas_auxiliar(atual->filhos[palavra[i] - VALOR_a], palavra, n, i + 1);

    //Caso i tenha chegado em n e atual tenha filhos, seguindo a recursão, entra no loop
    if (retorno)
    {
        //libera os filhos específicos (cada letra)
        free(atual->filhos[palavra[i] - VALOR_a]);
        atual->filhos[palavra[i] - VALOR_a] = NULL;

        //checa se ainda ha filhos, se tiver retorna falso e para a recursao, caso contrario, retira completamente o no e segue a recursao
        for (int j = 0; j < LETRAS; j++)
        {
            if (atual->filhos[j])
            {
...
void excluirTodas(PONT raiz, char* palavra, int n){
    ...
    excluirTodas_auxiliar(raiz, palavra, n, 0);
...
void excluir_auxiliar(PONT atual, char* palavra, int n, int i){
    ...
    if (atual->filhos && palavra[i])
    {
        //checa se tem o filho/letra especifico buscado
        if (atual->filhos[(palavra[i] - VALOR_a)])
        {
            //assinala-se o proximo como um filho especifico (proxima letra) da palavra
            PONT prox = atual->filhos[(palavra[i] - VALOR_a)];
            //aumenta o index para procurar pela proxima letra na palavra
            i++;

            excluir_auxiliar(prox, palavra, n, i);
...
void excluir(PONT raiz, char* palavra, int n){
    ...
    //contador recebe a quantidade de vezes que aparece a palavra passada
    int contador = buscarPalavra(raiz, palavra, n);

    /*
    Avalia os três casos possíveis, se já não houver a palavra, não faz nada, se só houver uma, apaga a palavra permanentemente,
    se tiver mais que uma, reduz em um o contador
    */
    if (contador == 0)
    {
        return;
    ...
    {
        excluir_auxiliar(raiz, palavra, n, 0);
    }
